...
exports.register = function register (rec) {
  ...
  const pairingMachineId = rcrypto.hkdf(otp, SALT, 'pairing-machine-id')
  const key = rcrypto.hkdf(otp, SALT, 'pairing-key')
...
exports.fetchDeviceRecord = function fetchDeviceRecord (machineId, otp) {
  const pairingDeviceId = rcrypto.hkdf(otp, SALT, 'pairing-device-id')
