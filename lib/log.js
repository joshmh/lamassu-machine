...
function init (machineId, categories, remoteIndexes, newLog$) {
  ...
  groupedRecs$.flatMap((g, i) =>
    g.skipWhile(r => r.index <= remoteIndexes[categories[i].code])
    .map(r => ({
      PutRequest: {
        Item: {
          MachineId: {S: machineId},
          SerialNumber: {N: r.index},
          Timestamp: {S: r.ts},
          Ciphertext: {B: r.data}
        }
      }
