...
Brain.prototype._processReal = function _processReal (req) {
  switch (req.button) {
    ...
    case 'tapDance':
      ...
      break
    case 'pairingCode':
      this._pairing(req.data)
      break
    case 'pairing':
      this._timedState('pairingCode', {timeout: 120000})
      break
  }
}
...
Brain.prototype._admin = function _admin () {
  ...
  browser.send({action: 'admin'})
  setTimeout(function () {
    self.adminEntry = false
    browser.send({action: 'unadmin'})
  }, 120000)
}

Brain.prototype._pairing = function _pairing (deviceCode) {
  const otp = pairing.generateOTP()
  const otpDisplay = pairing.prettyPrintOTP(otp)
  this._timedState('pairing', {data: {pairing: otpDisplay}})
  pairing.register({
    otp: otp,
    machineId: machineId,
  ...
  .subscribe(
    r => this.browser().send({pairing: r}),
    err => console.log(err.stack),
    this._timedState('paired')
  )
}
