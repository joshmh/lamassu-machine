...
var Brain = function (config) {
  ...
  this.creditConfirmed = {fiat: 0, cryptoAtoms: new BigNumber(0)}
  this.fiatTx = null
  this.pending = null
...
Brain.prototype._idle = function _idle (locale) {
  ...
  if (this.trader.twoWayMode && this.cartridges()) {
    this._ensureRaqiaRegistration(function () {
      self._idleTwoWay(self.localeInfo)
    })
  } else {
...
Brain.prototype.startScreen = function startScreen () {
  ...
  if (this.cryptoCode === 'BTC' && this.trader.twoWayMode && this.cartridges()) {
...
Brain.prototype._chooseCoinScreen = function _chooseCoinsScreen (localeInfo, cartridges) {
  if (this.singleCrypto()) {
    this.browser().send({localeInfo: localeInfo, cartridges: cartridges, coins: this.trader.coins})
    return this._chooseCoin(this.trader.coins[0])
  }

  this._transitionState('chooseCoin', {localeInfo: localeInfo, cartridges: cartridges, coins: this.trader.coins})
}

Brain.prototype.isIdleState = function isIdleState () {
  return this.singleCrypto()
...
Brain.prototype.balance = function balance () {
  ...
  console.log('DEBUG3')
  console.log(this.trader.balances)
...
Brain.prototype._firstBill = function _firstBill () {
  ...
  this._logTx({sessionId: this.trader.sessionId, cryptoAddress: address},
    'scanAddress')
}
...
Brain.prototype._billValid = function _billValid () {
  ...
  tradeRec.toAddress = this.cryptoAddress
  tradeRec.partialTx = _.clone(this.credit)

  // backwards compatibility
  tradeRec.satoshis = tradeRec.cryptoAtoms
  tradeRec.partialTx.satoshis = tradeRec.partialTx.cryptoAtoms

  this.trader.trade(tradeRec, function (err) {
    if (!err) {
      self.creditConfirmed.fiat += pending.fiat
      self.creditConfirmed.cryptoAtoms = self.creditConfirmed.cryptoAtoms.plus(pending.cryptoAtoms)
    }
  })
  ...
  var rec = {
    bill: pending.fiat,
    currency: this.currency,
    coinType: this.cryptoCode,
    cryptoAtoms: pending.cryptoAtoms
  }
...
Brain.prototype._createPendingTransaction = function _createPendingTransaction (bill) {
  ...
  console.log('DEBUG1')
  console.log(new BigNumber(bill).div(exchangeRate).mul(unitScaleFactor).toString())
...
Brain.prototype._executesendCoins = function _executesendCoins () {
  ...
  this._logTx(rec, 'cryptosRequested')
  ...
  this.trader.sendCoins(tx, function (err, transactionId) {
    ...
    if (err && unconfirmedFiat > 0) self._sendCoinsError(err)
...
Brain.prototype._cashInComplete = function _cashInComplete () {
  ...
  var rec = {
    cryptoAtoms: this.credit.cryptoAtoms,
    fiat: this.credit.fiat,
    currency: this.currency
  }
  this._logTx(rec, 'cryptosSent')
...
Brain.prototype._resetState = function _resetState () {
  ...
  this.creditConfirmed.cryptoAtoms = new BigNumber(0)
  this.pending = null
}
