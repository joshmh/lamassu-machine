...
var config$ = raqiaClient.stream({interval: 5000})
...
Brain.prototype._initLog = function initLog () {
  ...
  transactions$.filter(r =>
    (r.data.event === 'fiat-phone' || r.data.event === 'fiat-dispense-request') &&
    r.data.phone
  )
  .subscribe(r => {
    console.log('DEBUG21')
    let sessions = phoneMap.get(r.data.phone) || new Set()
    sessions.add(r.data.sessionId)
    phoneMap.set(r.data.phone, sessions)
  })

  // Maintain sessionMap
  transactions$
  .subscribe(r => sessionMap.set(r.data.sessionId, r))

  console.log('DEBUG25')

  // Manage confirmations
  transactions$.filter(r => r.data.event === 'fiat-dispense-request')
  .subscribe(r => {
    console.log('DEBUG22')
    pendingTxs.add(r.data.sessionId)
  })

  transactions$.filter(r => r.data.event === 'fiat-confirmed')
  .subscribe(r => {
    console.log('DEBUG28')
    confirmedMap.set(r.data.sessionId, true)
    pendingTxs.delete(r.data.sessionId)
  })
  ...
  .subscribe(r => {
    ...
    console.log('DEBUG23')
    let tx = sessionMap.get(r).data
    console.dir(tx)
    tx.blockchainStatus = 'confirmed'
    tx.event = 'fiat-confirmed'
  ...
  // Manage dispenses
  transactions$.filter(r => r.data.event === 'fiat-dispense')
  .subscribe(r => {
    console.log('DEBUG43')
    console.dir(r)
    if (r.data.phone) {
      console.log('DEBUG83')
      let sessions = phoneMap.get(r.data.phone)
      if (sessions) {
        sessions.delete(r.data.sessionId)
        phoneMap.set(r.data.phone, sessions)
      }
    }
    dispenseMap.set(r.data.sessionId, true)
  })
...
Brain.prototype._doSendBitcoins = function _doSendBitcoins () {
  ...
  let send$ = wallet.send(tx.toAddress, tx.satoshis)
  send$.subscribe(
...
Brain.prototype._sendBitcoinsError = function _sendBitcoinsError (err) {
  ...
  this.log('transactions', {
    event: 'token-send',
    sessionId: this.trader.sessionId,
    tokenValue: this.credit.satoshis,
    fiatValue: this.credit.fiat,
    address: this.bitcoinAddress,
    currency: this.currency,
...
Brain.prototype._processPhoneNumber = function _processPhoneNumber (number) {
  if (this.redeem) {
    ...
    .filter(r => r.data.blockchainStatus === 'confirmed')[0]

    this.fiatTx = convertFromDB(tx)
  }
...
Brain.prototype._redeemDispense = function _redeemDispense () {
  ...
  console.log('DEBUG82')
  console.dir(this._effectiveBills())
  console.dir(tx)
...
Brain.prototype._waitForDispense = function _waitForDispense () {
  var tx = this.fiatTx
  var transactionStatus$ = blockchain.transactionStatus(tx.toAddress, tx.satoshis)
  console.log('DEBUG33')
...
function dispenseAmount (bills, cartridges) {
...
Brain.prototype._physicalDispense = function _physicalDispense (tx) {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    var dispensed = dispenseAmount(bills, cartridges)

    console.log('DEBUG11: %s', tx.toAddress)
    self.log('transactions', {
      event: 'fiat-dispense',
      sessionId: tx.sessionId,
      address: tx.toAddress,
    ...
    var wasFullDispense = dispensed === tx.fiat
