...
Brain.prototype._initLog = function initLog () {
  ...
  fullLog$.tap(console.log).filter(r =>
    r.category === 'transactions' &&
    (r.data.event === 'fiat-phone' || r.data.event === 'fiat-dispense-request') &&
    r.data.phone
  )
  .subscribe(r => {
    let arr = phoneMap.get(r.data.phone) || []
    if (!arr.some(rr => r.data.sessionId === rr.data.sessionId)) {
      arr.push(r)
      phoneMap.set(r.data.phone, arr)
    }
  })

  fullLog$.filter(r =>
    r.category === 'transactions' &&
    r.data.event === 'fiat-dispense'
  )
  .subscribe(r => {
    console.log('DEBUG43')
    console.dir(r)
    if (r.data.phone) {
      ...
      if (sessions) {
        let updatedSessions = sessions.filter(rr => rr.data.sessionId !== r.data.sessionId)
        phoneMap.set(r.data.phone, updatedSessions)
      }
    }
...
Brain.prototype._processPhoneNumber = function _processPhoneNumber (number) {
  if (this.redeem) {
    var sessionIds = phoneMap.get(number)
    var tx = sessionIds[0]
    if (!tx) return this._timedState('unknownPhoneNumber')
