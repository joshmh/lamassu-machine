...
function setMachineId () {
  if (!masterKey) return
  machineId = uuid.unparse(rcrypto.hkdf(masterKey, 'machine-id'))
}
...
Brain.prototype._initLog = function initLog () {
  let cats = [
    {code: 'activities', path: path.resolve(this.dataPath, 'activities.dat'), tableName: 'Activities'},
  ...
  .filter(r => R.contains(r.event, ['fiat-reset', 'fiat-dispense', 'fiat-rejected']))
  .scan((acc, r) => {
  ...
  .filter(r => R.contains(r.event, ['fiat-dispense-request', 'fiat-dispense', 'fiat-cancel']))
  .scan((acc, r) => {
  ...
  .map((arr) => R.zipWith(R.subtract, arr[0], arr[1]))
  .map(r => r.map(R.max(0)))
  .startWith([0, 0])
  ...
  newTransactions$.filter(r => R.contains(r.event,
    ['note-inserted', 'fiat-dispense-request']))
...
Brain.prototype.setKeypair = function setKeypair (data) {
  console.log('DEBUG29')
  const privKeyEncKey = rcrypto.hkdf(masterKey, 'private-key-encryption-key')
  privkey = rcrypto.decrypt(data.privKeyEnc, privKeyEncKey)
  pubkey = ursa.createPrivateKey(privkey).toPublicPem()
}
...
Brain.prototype._admin = function _admin () {
  if (!R.contains(this.state, STATIC_STATES)) return
...
Brain.prototype._pairing = function _pairing () {
  ...
  const otp = rcrypto.hash(rcrypto.sign(nonce, privkey))
  pairing.register({
    machineId: machineId,
    pubkey: pubkey,
    privkey: privkey,
  ...
  .then(() => {
    ...
    this._transitionState('pairing', {pairingMnemonic: mnemonic})
    return pairing.fetchDeviceRecord(machineId, otp, privkey)
  })
  .then(ltk => {
    // Send signed ltk back
    // verify user code as below
    return pairing.sendSignedLtk(ltk, privkey)
    .then(() => {
      const hash = rcrypto.hash(ltk)
...
Brain.prototype._initialize = function _initialize (code) {
  this._transitionState('initialize')
  ...
  const privKey = ursa.generatePrivateKey(4096).toPrivatePem()
  const privKeyEncKey = rcrypto.hkdf(masterKey, 'private-key-encryption-key')
  ...
  this.browser().send({masterKey: mnemonic})
}
