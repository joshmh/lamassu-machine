...
//  this.billValidator = require('./id003/id003').factory(config.id003); DEBUG
  this.billValidator = require('./mei/cashflow_sc').factory(config.id003);
...
Brain.prototype.run = function run() {
  ...
  setInterval(function() {
    var elapsed = Date.now() - self.bootTime;
    if (elapsed > exitTime)
      console.log('Time to shutdown. Waiting for an opportunity.');
    if (!_.contains(STATIC_STATES, self.state)) return;
...
Brain.prototype._cancelIdScan = function _cancelIdScan() {
  ...
  this._transitionState('fakeIdle');
  this.scanner.cancel();
};
...
Brain.prototype._billRead = function _billRead(data) {
  ...
  console.dir([totalFiat, this.credit.fiat, this.pending.fiat]); //DEBUG
