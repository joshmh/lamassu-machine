...
var Brain = function(config) {
  ...
  this.raqia = config.mockApi ?
    require('./mocks/raqia') :
    require('./raqia');
  this.raqia.configure(config.raqia);
...
Brain._ensureRaqiaRegistration = function _ensureRaqiaRegistration(cb) {
  var self = this;
  var raqiaPath = path.resolve(this.dataPath, 'raqia.json');
  if (this.raqia.isRegistered()) return cb();

  fs.readFile(raqiaPath, function(err, res) {
    if (!err) {
      self.raqia.register(JSON.parse(res));
      return cb();
    }

    self.trader.registerRaqia(function(err, res) {
      if (err) {
        console.log(err);
        process.exit(1);
      }
...
Brain.prototype._idle = function _idle() {
  ...
  this._ensureRaqiaRegistration(function() {
    if (self.trader.twoWayMode) self._idleTwoWay();
    else self._idleOneWay();
  });
};
