...
Brain.prototype._ensureRaqiaRegistration = function _ensureRaqiaRegistration(cb) {
  var self = this;
  var raqiaPath = path.resolve(this.dataPath, 'raqia.json');
...
Brain.prototype._sendSecurityCode = function _sendSecurityCode(number) {
  ...
  this.idVerify.verifyPhoneUser(number, function(err, result) {
    if (err && err instanceof BadPhoneNumberError) {
...
Brain.prototype._securityCode = function _securityCode(code) {
  var self = this;
  var phone = this.currentPhoneNumber;

  if (!code) return this._idle();

  if (!this.currentSecurityCode) {
    this._transitionState('verifyingCode');
    this.idVerify.verifySms(phone, code, function(err, res) {
      if (err) { return self._timedState('id_verification_error_state'); }
      return self._phoneValid(res.success && phone);
    });
    return;
  }
};

Brain.prototype._phoneValid = function _phoneValid(phone) {
...
Brain.prototype._dispense = function _dispense() {
  ...
  this.raqia.dispense(sessionId, function(err) {
    ...
    if (err) return self._fiatError();
    self._physicalDispense(self.fiatTx.fiat, self.fiatTx.currencyCode);
  });
};

Brain.prototype._physicalDispense = function _physicalDispense(fiat, currency) {
  var self = this;
...
Brain.prototype._dispenseUpdate = function _dispenseUpdate(tx) {
  ...
  switch(status) {
    ...
    case 'authorized':
    case 'confirmed':
      if (status === 'authorized' && overZeroConfLimit) {
        this._timedState('redeemLater');
        break;
      }
      this._dispense();
