...
Brain.prototype._initLog = function initLog () {
  let cats = [
    ...
    {code: 'phoneRecords', path: path.resolve(this.dataPath, 'phone-records.dat'), tableName: 'PhoneRecords'},
  ...
  fullLog$.filter(R.prop('category', 'phoneRecords'))
  .subscribe(r => {
    if (r.record.action === 'add') phoneMap.set(r.record.phone, r.record.tx)
    if (r.record.action === 'expire') phoneMap.delete(r.record.phone)
  })

  fullLog$.filter(r =>
    r.category === 'transactions' &&
    r.event === 'fiat-dispense'
  )
  .subscribe(r => dispenseMap.set(r.sessionId, true))
}
...
Brain.prototype._waitForDispense = function _waitForDispense () {
  blockchain.waitForDispense()
  .then(tx => this._dispenseUpdate(tx))
  .catch(() => this._timedState('depositTimeout'))
}
