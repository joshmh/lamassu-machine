...
Brain.prototype._initLog = function initLog () {
  ...
  // Manage dispenses
  transactions$.filter(r => r.event === 'fiat-dispense')
  .subscribe(r => {
    console.log('DEBUG43')
    console.dir(r)
    if (r.phone) {
...
Brain.prototype._redeemDispense = function _redeemDispense () {
  ...
  tx.bills = BillMath.makeChange(this._effectiveBills(), tx.fiatValue)
...
Brain.prototype._cashOut = function _cashOut () {
  ...
  tx.bills = BillMath.makeChange(effectiveBills, tx.fiatValue)
...
Brain.prototype._physicalDispense = function _physicalDispense (tx) {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    console.log('DEBUG11: %s', tx.address)
