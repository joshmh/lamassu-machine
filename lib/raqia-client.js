...
exports.registerPairing = function registerPairing (rec) {
  ...
  console.log(params) // DEBUG00
  return putItem(params)
}

function privateDecrypt (item, key, privkey) {
  const encryptedSessionKey = item.SessionKey.B
  const sessionKey = rcrypto.privateDecrypt(encryptedSessionKey, privkey)
  return rcrypto.decrypt(toCryptoRecord(item), sessionKey)
}

exports.fetchPairing = function fetchDevice (machineId, pairingId, key, privkey) {
  const params = {
    TableName: 'Pairing',
    Key: {Id: {S: pairingId}}
  }
  return getItem(params)
  .map(r => r.Item ? privateDecrypt(r.Item, key, privkey) : null)
}
