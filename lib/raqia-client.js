...
exports.fetchPairing = function fetchDevice (machineId, pairingId, key) {
  const params = {
    TableName: 'Pairing',
    Key: {
  ...
  return getItem(params)
  .map(r => r.Item ? rcrypto.decrypt(toCryptoRecord(r.Item), key) : null)
}
...
function deriveBroadcastKeyGenerator (rootKey, gen) {
  return function deriveBroadcastKey (subject) {
    return rcrypto.hkdf(rootKey, gen.toString() + ':broadcast-key:' + subject)
  }
}

function computeEncryptionKeysGeneration (deviceId, rootKey, gen) {
  const _deriveBroadcastKey = deriveBroadcastKeyGenerator(rootKey, gen)
  const directKey = rcrypto.hkdf(rootKey, gen.toString() + ':direct-key:' + deviceId)
  const broadcastKeys = R.zipObj(KEY_SUBJECTS.map(r => [r, _deriveBroadcastKey(r)]))
  return R.merge({direct: directKey}, broadcastKeys)
}

function computeEncryptionKeys (deviceId, rootKey, gen) {
  const minGen = Math.max(0, gen - KEY_HISTORY_GENERATIONS + 1)
  let encryptionKeys = {}
  for (let i = minGen; i <= gen; i++) {
    let genKeys = computeEncryptionKeysGeneration(deviceId, rootKey, i)
    encryptionKeys = R.merge(encryptionKeys, genKeys)
  }
}

exports.publishKeys = function publishKeys (machineId, devicePublicKey, rootKey, gen) {
  ...
  const encryptionKeys = computeEncryptionKeys(deviceId, rootKey, gen)
