...
exports.putKeys = function putKeys (rec) {
  const key = rcrypto.generateKey()
  const encryptedKey = rcrypto.publicEncrypt(key, rec.devicePublicKey)
  const cipherRec = rcrypto.encrypt(rec.encryptionKeys, key)
  const params = {
    TableName: 'EncryptionKeys',
    Item: R.merge(cipherRec, {
      MachineId: {S: rec.machineId},
      DeviceId: {S: rec.deviceId},
