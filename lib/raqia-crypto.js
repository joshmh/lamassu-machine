...
var RSA_KEY_LENGTH = 3072 // See http://www.keylength.com/en/compare/
...
function encrypt (buf, key) {
  ...
  var len = 2 + 16 + 12 + buf.length
  return Buffer.concat([VERSION_BUF, tag, iv, encrypted], len)
}

function decrypt (enc, key) {
  var rawKey = extractRawKey(key)
  var tag = enc.slice(2, 18)
  var iv = enc.slice(18, 30)
  var content = enc.slice(30)
  var decipher = crypto.createDecipheriv(ALGO, rawKey, iv)
...
// *id* is the serial number of the key, up to 2 bytes
function generateKey (generation) {
  var key = new Buffer(26)
  key.writeUInt16LE(generation)
  crypto.randomBytes(24).copy(key, 2)
  return key
}
