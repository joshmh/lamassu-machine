...
function tradeRequest (rec) {
  ...
  if (rec.direction === 'buy') {
    return {
      ...
      json: true,
      body: R.merge({
        amount: (rec.tokenValue / 1e8).toFixed(8),
...
exports.run = function run (trades$, config$) {
  ...
  .scan(([acc], [trade, price, config]) => {
    let tokenValue = trade.tokenValue
    let satoshiResult = acc + tokenValue
  ...
  .filter(r => r[1] !== null)
  .map(r => ({direction: 'buy', price: r[3], tokenValue: r[2], config: r[4]}))
  .map(tradeRequest)
...
let http$ = exports.run(trades$, config$)
