...
var CashflowSc = function(config) {
  ...
  this.denominations = denominationsTable[this.currency];
};
...
CashflowSc.prototype.hasDenominations = function hasDenominations() {
  return this.denominations ? true : false;
};
...
CashflowSc.prototype._parse = function _parse(packet) {
  ...
  if (status === 'billRead') {
    ...
    return process.nextTick(function() {
      if (result.bill.currencyCode !== self.config.currency)
        return self.reject();
      self.emit('billRead', result.bill.denomination);
    });
  }
...
CashflowSc.prototype._poll = function _poll() {
  console.log(this.enabledDenominations.toString(16));
...
function parseExtended(frame) {
  ...
  return {
    status: status,
    bill: {denomination: denomination, currencyCode: currencyCode}
  };
}
