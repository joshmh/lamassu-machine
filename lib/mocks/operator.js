/*
- Config state
- Use AWS SDK
- Receive transactions
- Receive stats
- Receive prices
*/

/*
  Notes:

  EncryptionKeys should be stored as a list, probably 3
  Include one extra field: expiration date
  When downloading and storing keys, store with expirationDate
  If past expirationDate, re-fetch keys
  Each key should have an id
  Each record should have a key id

  Machine should re-encrypt config before key expiration date
*/

var crypto = require('crypto')
var Promise = require('bluebird')
var minimist = require('minimist')
var AWS = require('aws-sdk')
var dynamodb = Promise.promisifyAll(new AWS.DynamoDB({
  apiVersion: '2012-08-10',
  region: 'us-east-1'
}))
var raqiaCrypto = require('../raqia-crypto')

var commandLine = minimist(process.argv.slice(2))
var currency = commandLine.fiat || 'EUR'

var EXPIRE_MS = 5 * 86400000

var pubkeys = {
  machine: {
    privkey: '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n',
    pubkey: '-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----\n'
  },
  device: {
    privkey: '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n',
    pubkey: '-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----\n'
  }
}

var state = {
  exchangeRate: 12.45,
  fiatExchangeRate: 1001.12,
  fiatTxLimit: 250,
  zeroConfLimit: 50,
  balance: 50,
  txLimit: null,
  idVerificationLimit: null,
  idVerificationEnabled: false,
  idData: null,
  isMock: true,
  locale: {
    currency: currency,
    localeInfo: {
      primaryLocale: 'en-US',
      primaryLocales: ['en-US', 'ja-JP', 'es-MX', 'he-IL', 'ar-SA'],
      country: 'US'
    }
  },
  twoWayMode: true,
  cartridges: [
    {denomination: 1, count: 2},
    {denomination: 5, count: 1}
  ],
  virtualCartridges: [100],
  cartridgesUpdateId: 12
}

var deviceId

function init (_deviceId) {
  deviceId = _deviceId
}

function fetchSymmetricKeys (machineId) {
  var params = {
    Key: {MachineId: {S: machineId}, DeviceId: {S: deviceId}},
    TableName: 'EncryptionKeys'
  }
  return dynamodb.getItemAsync(params)
  .then(function (res) {
    var broadcastEnc = res.Item.BroadcastKeys.BS[0]
    var directEnc = res.Item.DirectKeys.BS[0]
    return {
      direct: raqiaCrypto.privateDecrypt(directEnc, pubkeys.device.privkey),
      broadcast: raqiaCrypto.privateDecrypt(broadcastEnc, pubkeys.device.privkey)
    }
  })
}

function updateConfig (config, key) {
  var date = new Date()
  var cipherRec = raqiaCrypto.encrypt(new Buffer(JSON.stringify(config)), key)
  var nonce = crypto.randomBytes(16)
  console.log(cipherRec)
  var params = {
    Item: {
      MachineId: {S: machineId},
      Timestamp: {S: date.toISOString()},
      Nonce: {B: nonce},
      Version: {N: cipherRec.version.toString()},
      Iv: {B: cipherRec.iv},
      Ciphertext: {B: cipherRec.ciphertext}
    },
    TableName: 'Config'
  }
  return dynamodb.putItemAsync(params)
}

function fetchConfig (key) {
  var params = {
    Key: {MachineId: {S: machineId}},
    TableName: 'Config'
  }
  return dynamodb.getItemAsync(params)
  .then(function (res) {
    var str = raqiaCrypto.decrypt(res.Item.Content.B, key)
    return JSON.parse(str)
  })
}

function pair (machineId) {
  var date = new Date()
  var devicePubkey = pubkeys.device.pubkey
  var expirationDate = new Date(date.getTime() + EXPIRE_MS)

  var broadcast = new Buffer('AABiOJ/+Ta92x95yeExTCTTm', 'base64')
  var direct = new Buffer('AAC1OmI3FYNo2ddXmsuu3LU3', 'base64')

  var broadcastEnc = raqiaCrypto.publicEncrypt(broadcast, devicePubkey)
  var directEnc = raqiaCrypto.publicEncrypt(direct, devicePubkey)
  var params = {
    Item: {
      MachineId: {S: machineId},
      DeviceId: {S: deviceId},
      Timestamp: {S: date.toISOString()},
      ExpirationDate: {S: expirationDate.toISOString()},
      BroadcastKeys: {BS: [broadcastEnc]},
      DirectKeys: {BS: [directEnc]}
    },
    TableName: 'EncryptionKeys'
  }
  return dynamodb.putItemAsync(params)
}

module.exports = {
  init: init
}

deviceId = 'ca077d8e-c72b-4000-b40b-61a7761ddda8'
var machineId = '990685c8-80e4-4344-b6d1-bd288f031b2f'

var broadcastKey = new Buffer([ 197, 132, 27, 114, 98, 251, 79, 5, 34, 77, 5, 157, 26, 215, 183, 72 ])
// pair(machineId).then(console.log)
// fetchSymmetricKeys(machineId).then(console.log)
updateConfig(state, broadcastKey).then(console.log)
// fetchConfig(broadcastKey).then(console.log)
