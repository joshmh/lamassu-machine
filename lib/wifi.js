...
function hasActiveInterfaces () {
  ...
  var interfaces = []
  _.forEach(networks, function (val, key) {
    var realIp = _.where(val, {family: 'IPv4', internal: false})

    // We ignore usb0, because that's set up for zeroconf in N7G1 inittab
    if (realIp.length === 1 && key !== 'usb0') interfaces.push(key)
  })
...
WiFi.prototype.status = function status (cb) {
  ...
  this.wpa.command('status', function (err, res) {
    ...
    switch (networkStatus) {
      ...
      // TODO could be temporary on startup, need more checks
...
WiFi.prototype._handleScan = function _handleScan (res) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var rec = {bssid: parsed[0], strength: strength, security: parsed[3],
      ssid: ssid, displaySsid: displaySsid, rawSsid: rawSsid}
    results.push(rec)
  }
...
WiFi.prototype._truncate = function _truncate (str, len) {
  if (str.length > len) {
    var actualLen = len - 2
    var beginLen = Math.floor(actualLen / 2)
    var endLen = actualLen - beginLen
    var endIndex = str.length - endLen
